...
var (
	tattooDBTypes = map[string]string{`ID`: `bigint`, `IDProfile`: `bigint`, `IDImage`: `bigint`, `Likes`: `integer`, `Description`: `text`, `Categories`: `ARRAYtext`, `CreatedAt`: `timestamp without time zone`, `Popularity`: `integer`, `IDPost`: `bigint`, `Views`: `integer`, `Coordinate`: `text`}
	_             = bytes.MinRead
)

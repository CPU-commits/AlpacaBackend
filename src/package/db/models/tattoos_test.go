...
func testTattoosInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(tattooColumnsWithoutDefault...)); err != nil {
		t.Error(err)
	}
...
func testTattooToManyIDTattooTattooCategories(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.IDTattooTattooCategories().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.IDTattoo == b.IDTattoo {
			bFound = true
		}
		if v.IDTattoo == c.IDTattoo {
			cFound = true
		}
	}
	...
	slice := TattooSlice{&a}
	if err = a.L.LoadIDTattooTattooCategories(ctx, tx, false, (*[]*Tattoo)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDTattooTattooCategories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDTattooTattooCategories = nil
	if err = a.L.LoadIDTattooTattooCategories(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDTattooTattooCategories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTattooToManyAddOpIDTattooTattooCategories(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDTattooTattooCategories(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDTattooTattooCategories[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDTattooTattooCategories[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDTattooTattooCategories().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	tattooDBTypes = map[string]string{`ID`: `int8`, `IDProfile`: `int8`, `IDImage`: `int8`, `Likes`: `int4`, `CreatedAt`: `timestamp`, `IDPost`: `int8`, `Description`: `string`, `Popularity`: `int4`, `Views`: `int4`, `Coordinate`: `geometry`}
	_             = bytes.MinRead
)

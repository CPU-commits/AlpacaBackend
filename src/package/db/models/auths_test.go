...
func testAuthsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(authPrimaryKeyColumns, authColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testAuthToManyIDAuthSessions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, authDBTypes, true, authColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Auth struct: %s", err)
	}
	...
	check, err := a.IDAuthSessions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.IDAuth == b.IDAuth {
			bFound = true
		}
		if v.IDAuth == c.IDAuth {
			cFound = true
		}
	}
	...
	slice := AuthSlice{&a}
	if err = a.L.LoadIDAuthSessions(ctx, tx, false, (*[]*Auth)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDAuthSessions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDAuthSessions = nil
	if err = a.L.LoadIDAuthSessions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDAuthSessions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testAuthToManyAddOpIDAuthSessions(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDAuthSessions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.IDAuth {
			t.Error("foreign key was wrong value", a.ID, first.IDAuth)
		}
		if a.ID != second.IDAuth {
			t.Error("foreign key was wrong value", a.ID, second.IDAuth)
		}

		if first.R.IDAuthAuth != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.IDAuthAuth != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.IDAuthSessions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDAuthSessions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDAuthSessions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testAuthToOneUserUsingIDUserUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadIDUserUser(ctx, tx, false, (*[]*Auth)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAuthToOneSetOpUserUsingIDUserUser(t *testing.T) {
	var err error
...
var (
	authDBTypes = map[string]string{`ID`: `bigint`, `Password`: `text`, `IDUser`: `bigint`, `CreatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)

...
func testTokenToOneUserUsingIDUserUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadIDUserUser(ctx, tx, false, (*[]*Token)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTokenToOneSetOpUserUsingIDUserUser(t *testing.T) {
	var err error
...
var (
	tokenDBTypes = map[string]string{`ID`: `bigint`, `Token`: `text`, `IDUser`: `bigint`, `IsActive`: `boolean`, `CreatedAt`: `timestamp without time zone`, `ExpiresAt`: `timestamp without time zone`}
	_            = bytes.MinRead
)

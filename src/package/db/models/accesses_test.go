...
func testAccessesUpsert(t *testing.T) {
	...
	if len(accessAllColumns) == len(accessPrimaryKeyColumns) {
...
func testAccessToOneSessionUsingIDSessionSession(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, accessDBTypes, false, accessColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Access struct: %s", err)
	}
	...
	local.IDSession = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.IDSessionSession().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	AddSessionHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *Session) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := AccessSlice{&local}
	if err = local.L.LoadIDSessionSession(ctx, tx, false, (*[]*Access)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.IDSessionSession == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.IDSessionSession = nil
	if err = local.L.LoadIDSessionSession(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.IDSessionSession == nil {
		t.Error("struct should have been eager loaded")
...
func testAccessToOneSetOpSessionUsingIDSessionSession(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, accessDBTypes, false, strmangle.SetComplement(accessPrimaryKeyColumns, accessColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Session{&b, &c} {
		err = a.SetIDSessionSession(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.IDSessionSession != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.IDSessionAccesses[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.IDSession != x.ID {
			t.Error("foreign key was wrong value", a.IDSession)
		}

		zero := reflect.Zero(reflect.TypeOf(a.IDSession))
		reflect.Indirect(reflect.ValueOf(&a.IDSession)).Set(zero)
		...
		if a.IDSession != x.ID {
			t.Error("foreign key was wrong value", a.IDSession, x.ID)
		}
	}
}
...
var (
	accessDBTypes = map[string]string{`ID`: `int8`, `Token`: `string`, `ExpiresAt`: `date`, `IsRevoked`: `bool`, `IDSession`: `int8`, `CreatedAt`: `timestamp`}
	_             = bytes.MinRead
)
...
func testAccessesSliceUpdateAll(t *testing.T) {
	...
	var fields []string
	if strmangle.StringSliceMatch(accessAllColumns, accessPrimaryKeyColumns) {
		fields = accessAllColumns
	} else {

...
func testPostToManyIDPostLikes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.IDPostLikes().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := PostSlice{&a}
	if err = a.L.LoadIDPostLikes(ctx, tx, false, (*[]*Post)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDPostLikes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDPostLikes = nil
	if err = a.L.LoadIDPostLikes(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDPostLikes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testPostToManyIDPostPostImages(t *testing.T) {
	...
	if err = randomize.Struct(seed, &b, postImageDBTypes, false, postImageColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, postImageDBTypes, false, postImageColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.IDPostPostImages().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := PostSlice{&a}
	if err = a.L.LoadIDPostPostImages(ctx, tx, false, (*[]*Post)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDPostPostImages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDPostPostImages = nil
	if err = a.L.LoadIDPostPostImages(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDPostPostImages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testPostToManyIDPostTattoos(t *testing.T) {
	...
	check, err := a.IDPostTattoos().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := PostSlice{&a}
	if err = a.L.LoadIDPostTattoos(ctx, tx, false, (*[]*Post)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.IDPostTattoos = nil
	if err = a.L.LoadIDPostTattoos(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDPostTattoos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testPostToManyAddOpIDPostLikes(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDPostLikes(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDPostLikes[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDPostLikes[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}
...
func testPostToManyAddOpIDPostPostImages(t *testing.T) {
	var err error
	...
	var a Post
	var b, c, d, e PostImage

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, postDBTypes, false, strmangle.SetComplement(postPrimaryKeyColumns, postColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*PostImage{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, postImageDBTypes, false, strmangle.SetComplement(postImagePrimaryKeyColumns, postImageColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*PostImage{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDPostPostImages(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDPostPostImages[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDPostPostImages[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}
...
func testPostToManyAddOpIDPostTattoos(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDPostTattoos(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDPostTattoos[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDPostTattoos[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDPostTattoos().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testPostToManySetOpIDPostTattoos(t *testing.T) {
	var err error
	...
	err = a.SetIDPostTattoos(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetIDPostTattoos(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.IDPostTattoos[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.IDPostTattoos[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testPostToManyRemoveOpIDPostTattoos(t *testing.T) {
	var err error
	...
	err = a.AddIDPostTattoos(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveIDPostTattoos(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.IDPostTattoos) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.IDPostTattoos[1] != &d {
...
func testPostToOneProfileUsingIDProfileProfile(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadIDProfileProfile(ctx, tx, false, (*[]*Post)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPostToOneStudioUsingIDStudioStudio(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := PostSlice{&local}
	if err = local.L.LoadIDStudioStudio(ctx, tx, false, (*[]*Post)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPostToOneSetOpProfileUsingIDProfileProfile(t *testing.T) {
	var err error
...
func testPostToOneSetOpStudioUsingIDStudioStudio(t *testing.T) {
	var err error
...
func testPostToOneRemoveOpStudioUsingIDStudioStudio(t *testing.T) {
	var err error
...
var (
	postDBTypes = map[string]string{`ID`: `bigint`, `IDProfile`: `bigint`, `IDStudio`: `bigint`, `IsStudioPost`: `boolean`, `Content`: `text`, `Likes`: `integer`, `Categories`: `ARRAY_text`, `Mentions`: `ARRAY_int4`, `Views`: `integer`, `CreatedAt`: `timestamp without time zone`, `Shares`: `integer`}
	_           = bytes.MinRead
)

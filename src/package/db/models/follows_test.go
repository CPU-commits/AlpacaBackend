...
func testFollowsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(followPrimaryKeyColumns, followColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testFollowToOneProfileUsingIDProfileProfile(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadIDProfileProfile(ctx, tx, false, (*[]*Follow)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testFollowToOneUserUsingIDUserUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := FollowSlice{&local}
	if err = local.L.LoadIDUserUser(ctx, tx, false, (*[]*Follow)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testFollowToOneSetOpProfileUsingIDProfileProfile(t *testing.T) {
	var err error

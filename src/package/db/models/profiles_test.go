...
func testProfileToManyIDProfileProfileCategories(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.IDProfileProfileCategories().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ProfileSlice{&a}
	if err = a.L.LoadIDProfileProfileCategories(ctx, tx, false, (*[]*Profile)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDProfileProfileCategories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDProfileProfileCategories = nil
	if err = a.L.LoadIDProfileProfileCategories(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDProfileProfileCategories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProfileToManyAddOpIDProfileProfileCategories(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDProfileProfileCategories(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDProfileProfileCategories[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDProfileProfileCategories[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDProfileProfileCategories().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	profileDBTypes = map[string]string{`ID`: `int8`, `Description`: `string`, `IDAvatar`: `int8`, `Likes`: `int4`, `IDUser`: `int8`, `CreatedAt`: `timestamp`}
	_              = bytes.MinRead
)

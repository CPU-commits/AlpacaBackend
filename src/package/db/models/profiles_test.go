...
func testProfileToManyIDProfileDesigns(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, designDBTypes, false, designColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, designDBTypes, false, designColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.IDProfileDesigns().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadIDProfileDesigns(ctx, tx, false, (*[]*Profile)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDProfileDesigns); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDProfileDesigns = nil
	if err = a.L.LoadIDProfileDesigns(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDProfileDesigns); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProfileToManyIDProfileViews(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.IDProfileViews().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ProfileSlice{&a}
	if err = a.L.LoadIDProfileViews(ctx, tx, false, (*[]*Profile)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDProfileViews); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDProfileViews = nil
	if err = a.L.LoadIDProfileViews(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDProfileViews); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProfileToManyAddOpIDProfileDesigns(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDProfileDesigns(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDProfileDesigns[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDProfileDesigns[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDProfileDesigns().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testProfileToManyAddOpIDProfileViews(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDProfileViews(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDProfileViews[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDProfileViews[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDProfileViews().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testProfileToManySetOpIDProfileViews(t *testing.T) {
	var err error
	...
	err = a.SetIDProfileViews(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetIDProfileViews(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.IDProfileViews[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.IDProfileViews[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testProfileToManyRemoveOpIDProfileViews(t *testing.T) {
	var err error
	...
	err = a.AddIDProfileViews(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveIDProfileViews(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.IDProfileViews) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.IDProfileViews[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.IDProfileViews[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

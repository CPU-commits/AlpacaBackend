...
func testSessionToManyIDSessionAccesses(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, accessDBTypes, false, accessColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, accessDBTypes, false, accessColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.IDSessionAccesses().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.IDSession == b.IDSession {
			bFound = true
		}
		if v.IDSession == c.IDSession {
			cFound = true
		}
	}
	...
	if err = a.L.LoadIDSessionAccesses(ctx, tx, false, (*[]*Session)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDSessionAccesses); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDSessionAccesses = nil
	if err = a.L.LoadIDSessionAccesses(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDSessionAccesses); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testSessionToManyAddOpIDSessionAccesses(t *testing.T) {
	var err error
	...
	foreigners := []*Access{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, accessDBTypes, false, strmangle.SetComplement(accessPrimaryKeyColumns, accessColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Access{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDSessionAccesses(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.IDSession {
			t.Error("foreign key was wrong value", a.ID, first.IDSession)
		}
		if a.ID != second.IDSession {
			t.Error("foreign key was wrong value", a.ID, second.IDSession)
		}

		if first.R.IDSessionSession != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.IDSessionSession != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.IDSessionAccesses[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDSessionAccesses[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDSessionAccesses().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testSessionToOneAuthUsingIDAuthAuth(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, authDBTypes, false, authColumnsWithDefault...); err != nil {
	...
	local.IDAuth = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.IDAuthAuth().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadIDAuthAuth(ctx, tx, false, (*[]*Session)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.IDAuthAuth == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.IDAuthAuth = nil
	if err = local.L.LoadIDAuthAuth(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.IDAuthAuth == nil {
		t.Error("struct should have been eager loaded")
...
func testSessionToOneSetOpAuthUsingIDAuthAuth(t *testing.T) {
	var err error
	...
	for i, x := range []*Auth{&b, &c} {
		err = a.SetIDAuthAuth(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if x.R.IDAuthSessions[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.IDAuth != x.ID {
			t.Error("foreign key was wrong value", a.IDAuth)
		}

		zero := reflect.Zero(reflect.TypeOf(a.IDAuth))
		reflect.Indirect(reflect.ValueOf(&a.IDAuth)).Set(zero)
		...
		if a.IDAuth != x.ID {
			t.Error("foreign key was wrong value", a.IDAuth, x.ID)
		}
	}
}

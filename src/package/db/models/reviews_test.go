...
func testReviewToOneProfileUsingIDProfileProfile(t *testing.T) {
	...
	slice := ReviewSlice{&local}
	if err = local.L.LoadIDProfileProfile(ctx, tx, false, (*[]*Review)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testReviewToOneUserUsingIDUserUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := ReviewSlice{&local}
	if err = local.L.LoadIDUserUser(ctx, tx, false, (*[]*Review)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testReviewToOneSetOpUserUsingIDUserUser(t *testing.T) {
	var err error
...
var (
	reviewDBTypes = map[string]string{`ID`: `bigint`, `IDUser`: `bigint`, `IDProfile`: `bigint`, `IDAppointment`: `bigint`, `Content`: `text`, `Stars`: `integer`, `CreatedAt`: `timestamp without time zone`}
	_             = bytes.MinRead
)

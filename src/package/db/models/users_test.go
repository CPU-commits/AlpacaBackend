...
func testUserToManyIDUserStudioUserHistories(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.IDUserStudioUserHistories().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadIDUserStudioUserHistories(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDUserStudioUserHistories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDUserStudioUserHistories = nil
	if err = a.L.LoadIDUserStudioUserHistories(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDUserStudioUserHistories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpIDUserStudioUserHistories(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDUserStudioUserHistories(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDUserStudioUserHistories[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDUserStudioUserHistories[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDUserStudioUserHistories().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Email`: `text`, `Name`: `text`, `Phone`: `text`, `CreatedAt`: `timestamp without time zone`, `Username`: `text`, `Location`: `text`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)

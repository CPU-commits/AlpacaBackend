...
var (
	appointmentDBTypes = map[string]string{`ID`: `bigint`, `IDUser`: `bigint`, `IDTattooArtist`: `bigint`, `Status`: `enum.appointment_status('scheduled','canceled','created','reviewed')`, `IDStudio`: `bigint`, `IDCalendar`: `text`, `Phone`: `text`, `HasIdea`: `boolean`, `HasDesign`: `boolean`, `Area`: `enum.tattoo_area('arm','leg','back','chest','abdomen','neck','head','hand','foot','hip','other')`, `Height`: `double precision`, `Width`: `double precision`, `Color`: `enum.tattoo_color('black','full_color')`, `IDDesign`: `bigint`, `Description`: `text`, `ScheduledAt`: `timestamp without time zone`, `Duration`: `double precision`, `FinishedAt`: `timestamp without time zone`, `IsPaid`: `boolean`, `CreatedAt`: `timestamp without time zone`}
	_                  = bytes.MinRead
)

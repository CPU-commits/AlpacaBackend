...
var (
	appointmentDBTypes = map[string]string{`ID`: `int8`, `IDUser`: `int8`, `IDTattooArtist`: `int8`, `Status`: `public.appointment_status`, `Phone`: `string`, `HasIdea`: `bool`, `Area`: `public.tattoo_area`, `Height`: `float8`, `Width`: `float8`, `Color`: `public.tattoo_color`, `Description`: `string`, `ScheduledAt`: `timestamp`, `Duration`: `float8`, `FinishedAt`: `timestamp`, `CreatedAt`: `timestamp`}
	_                  = bytes.MinRead
)

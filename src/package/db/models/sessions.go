...
type Session struct {
	...
	IDAuth    int64       `boil:"id_auth" json:"id_auth" toml:"id_auth" yaml:"id_auth"`
...
var SessionWhere = struct {
	...
	IDAuth:    whereHelperint64{field: "\"sessions\".\"id_auth\""},
	Device:    whereHelpernull_String{field: "\"sessions\".\"device\""},
	IP:        whereHelpernull_String{field: "\"sessions\".\"ip\""},
	Browser:   whereHelpernull_String{field: "\"sessions\".\"browser\""},
	Location:  whereHelpernull_String{field: "\"sessions\".\"location\""},
...
var SessionRels = struct {
	IDAuthAuth        string
	IDSessionAccesses string
}{
	IDAuthAuth:        "IDAuthAuth",
	IDSessionAccesses: "IDSessionAccesses",
}

// sessionR is where relationships are stored.
type sessionR struct {
	IDAuthAuth        *Auth       `boil:"IDAuthAuth" json:"IDAuthAuth" toml:"IDAuthAuth" yaml:"IDAuthAuth"`
	IDSessionAccesses AccessSlice `boil:"IDSessionAccesses" json:"IDSessionAccesses" toml:"IDSessionAccesses" yaml:"IDSessionAccesses"`
}
...
func (r *sessionR) GetIDAuthAuth() *Auth {
	if r == nil {
		return nil
	}
	return r.IDAuthAuth
...
func (r *sessionR) GetIDSessionAccesses() AccessSlice {
	if r == nil {
		return nil
	}
	return r.IDSessionAccesses
}
...
var (
	sessionAllColumns            = []string{"id", "token", "id_auth", "device", "ip", "browser", "location", "expires_at", "created_at"}
	sessionColumnsWithoutDefault = []string{"token", "id_auth", "expires_at"}
	sessionColumnsWithDefault    = []string{"id", "device", "ip", "browser", "location", "created_at"}
...
// IDAuthAuth pointed to by the foreign key.
func (o *Session) IDAuthAuth(mods ...qm.QueryMod) authQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"id\" = ?", o.IDAuth),
	}
...
// IDSessionAccesses retrieves all the access's Accesses with an executor via id_session column.
func (o *Session) IDSessionAccesses(mods ...qm.QueryMod) accessQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"accesses\".\"id_session\"=?", o.ID),
	)

	return Accesses(queryMods...)
}

// LoadIDAuthAuth allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (sessionL) LoadIDAuthAuth(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSession interface{}, mods queries.Applicator) error {
	var slice []*Session
	var object *Session
	...
	query := NewQuery(
		qm.From(`auths`),
		qm.WhereIn(`auths.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		object.R.IDAuthAuth = foreign
		if foreign.R == nil {
			foreign.R = &authR{}
		}
		foreign.R.IDAuthSessions = append(foreign.R.IDAuthSessions, object)
		return nil
	}

	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.IDAuth == foreign.ID {
				local.R.IDAuthAuth = foreign
				if foreign.R == nil {
					foreign.R = &authR{}
				}
				foreign.R.IDAuthSessions = append(foreign.R.IDAuthSessions, local)
				break
			}
		}
	}
...
// LoadIDSessionAccesses allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (sessionL) LoadIDSessionAccesses(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSession interface{}, mods queries.Applicator) error {
	var slice []*Session
	var object *Session
	...
	query := NewQuery(
		qm.From(`accesses`),
		qm.WhereIn(`accesses.id_session in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load accesses")
	}

	var resultSlice []*Access
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice accesses")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on accesses")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for accesses")
	}
	...
	if singular {
		object.R.IDSessionAccesses = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &accessR{}
			}
			foreign.R.IDSessionSession = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.IDSession {
				local.R.IDSessionAccesses = append(local.R.IDSessionAccesses, foreign)
				if foreign.R == nil {
					foreign.R = &accessR{}
				}
				foreign.R.IDSessionSession = local
				break
			}
		}
	}
...
// SetIDAuthAuth of the session to the related item.
// Sets o.R.IDAuthAuth to related.
// Adds o to related.R.IDAuthSessions.
func (o *Session) SetIDAuthAuth(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Auth) error {
	var err error
	if insert {
	...
	if o.R == nil {
		...
		o.R.IDAuthAuth = related
	}

	if related.R == nil {
		related.R = &authR{
			IDAuthSessions: SessionSlice{o},
		}
	} else {
		related.R.IDAuthSessions = append(related.R.IDAuthSessions, o)
	}

	return nil
}

// AddIDSessionAccesses adds the given related objects to the existing relationships
// of the session, optionally inserting them as new records.
// Appends related to o.R.IDSessionAccesses.
// Sets related.R.IDSessionSession appropriately.
func (o *Session) AddIDSessionAccesses(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Access) error {
	var err error
	...
	if o.R == nil {
		o.R = &sessionR{
			IDSessionAccesses: related,
		}
	} else {
		o.R.IDSessionAccesses = append(o.R.IDSessionAccesses, related...)
	}
...
func (o *Session) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryCockroachDB(dialect, "\"sessions\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return fmt.Errorf("models: unable to upsert sessions: %w", err)
	}

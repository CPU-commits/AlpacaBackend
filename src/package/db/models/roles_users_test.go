...
func testRolesUsersExists(t *testing.T) {
	...
	e, err := RolesUserExists(ctx, tx, o.ID)
	if err != nil {
...
func testRolesUsersFind(t *testing.T) {
	...
	rolesUserFound, err := FindRolesUser(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
func testRolesUsersInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(rolesUserPrimaryKeyColumns, rolesUserColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testRolesUserToOneUserUsingIDUserUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, rolesUserDBTypes, false, rolesUserColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize RolesUser struct: %s", err)
	}
	...
	if err = local.L.LoadIDUserUser(ctx, tx, false, (*[]*RolesUser)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRolesUserToOneSetOpUserUsingIDUserUser(t *testing.T) {
	var err error
...
var (
	rolesUserDBTypes = map[string]string{`ID`: `bigint`, `IDUser`: `bigint`, `Role`: `enum.role_name('admin','user','tattooArtist')`, `CreatedAt`: `timestamp without time zone`}
	_                = bytes.MinRead
)
